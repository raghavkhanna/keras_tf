...
# - load and compile the LeNet architecture
# - train the network
# - save the serialized network to disk for reuse
# - visualize examples of the network output
...
ap.add_argument("-l", "--load-model", type=int, default=-1,
                help="(optinal) load pretrained model?(-1/1)")
ap.add_argument("-w", "--weights", type=str,
                help="(optional) path to weights file", default="lenet_weights.hdf5")
...
data = data[:, np.newaxis, :, :]  # add 1 channel to the image for keras
(trainData, testData, trainLabels, testLabels) = train_test_split(
	data / 255.0, dataset.target.astype("int"), test_size=0.80)
